...
class NPA(geo_model.GeoModel):
    ...
    def test_func(self, cursor, uid, ids):
        ...
        print a.the_geom.wkt
        b = self.create(
            cursor, uid, {'name': '1100', 'city': 'lausanne', 'the_geom': a.the_geom})
        b = self.browse(cursor, uid, b)
        #self.unlink(cursor, uid, [b.id])
        view_id = self.pool.get('ir.ui.view').search(
            cursor, uid, [('model', '=', 'dummy.zip'), ('type', '=', 'geoengine')])[0]
        import pprint
        pprint.pprint(self.fields_view_get(cursor, uid, view_id=view_id,
                                           view_type='geoengine', context=None, toolbar=False, submenu=False))
