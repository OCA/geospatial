odoo.define('web_view_leaflet_map.MapRenderer', function (require) {
    ...
    var MapRenderer = AbstractRenderer.extend({
        ...
        _prepareMarkerIcon: function(record) {
            const iconAttrs = {
                iconUrl: "/web_view_leaflet_map/static/lib/leaflet/images/marker-icon.png",
                iconSize: [30, 45],
                popupAnchor: [0, -20],
            }
            if (this.field_marker_icon_image) {
                iconAttrs.iconUrl = session.url('/web/image', {
                    model: record.model,
                ...
                iconAttrs.className = "leaflet_marker_icon";
                iconAttrs.iconSize = [this.marker_icon_size_x, this.marker_icon_size_y];
                iconAttrs.popupAnchor = [this.marker_popup_anchor_x, this.marker_popup_anchor_y];
            }
            return L.icon(iconAttrs);
        },
        ...
        _preparePopUpData: function (record) {
            return (
                ...
                + "<h3>" + record.data[this.field_title] + "</h3>"
                + "<h5 class='fw-normal'>" + record.data[this.field_address] + "</h5>"
                + "</div>"
            );
        },
